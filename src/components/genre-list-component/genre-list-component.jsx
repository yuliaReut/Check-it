...
const GenreListComponent = () => {
  ...
  return <React.Fragment>
    <ul className="catalog__genres-list">
      <li className="catalog__genres-item catalog__genres-item--active" onClick={handleGenreClick} data-filter={Genres.all}>
        <a href="#" className="catalog__genres-link">{Genres.all}</a>
      </li>
      {
        genresList.map((genre, index) =>
          <li className="catalog__genres-item" key={`${genre}-${index}`} data-filter={genre} onClick={handleGenreClick} >
